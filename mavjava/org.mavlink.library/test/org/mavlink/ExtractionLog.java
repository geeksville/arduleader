...
public class ExtractionLog {
  ...
  public static void main(String[] args) {
    ...
    try {
      PrintWriter attitude = new PrintWriter(new BufferedWriter(new FileWriter(
          filename + "-attitude_(" + hdeb + "_" + mdeb + "_" + sdeb + "-"
              + hfin + "_" + mfin + "_" + sfin + ").csv")));
      PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter(
          filename + "-output_(" + hdeb + "_" + mdeb + "_" + sdeb + "-" + hfin
              + "_" + mfin + "_" + sfin + ").csv")));
      PrintWriter input = new PrintWriter(new BufferedWriter(new FileWriter(
          filename + "-input_(" + hdeb + "_" + mdeb + "_" + sdeb + "-" + hfin
              + "_" + mfin + "_" + sfin + ").csv")));

      DataInputStream dis = new DataInputStream(new FileInputStream(filename));
      // reader = new MAVLinkReader(dis,
      // IMAVLinkMessage.MAVPROT_PACKET_START_V09);
      ...
      while (dis.available() > 0) {
        ...
        if (msg != null
            && msg.messageType == IMAVLinkMessageID.MAVLINK_MSG_ID_ATTITUDE
          ...
          attitude.println(((msg_attitude) msg).roll + ";"
              + ((msg_attitude) msg).pitch + ";" + ((msg_attitude) msg).yaw
              + ";" + ((msg_attitude) msg).rollspeed + ";"
              + ((msg_attitude) msg).pitchspeed + ";"
              + ((msg_attitude) msg).yawspeed + ";"
              + ((double) (timeAttitude - lastTimeAttitude) / 1000));
        }
        if (msg != null
            && msg.messageType == IMAVLinkMessageID.MAVLINK_MSG_ID_SERVO_OUTPUT_RAW
            && ((((msg_servo_output_raw) msg).time_usec / 1000 >= debut && ((msg_servo_output_raw) msg).time_usec / 1000 <= fin) || !limitation)) {
          if (firstOutput) {
            lastTimeOutput = ((msg_servo_output_raw) msg).time_usec / 1000;
            timeOutput = ((msg_servo_output_raw) msg).time_usec / 1000;
            firstOutput = false;
          } else {
            lastTimeOutput = timeOutput;
            timeOutput = ((msg_servo_output_raw) msg).time_usec / 1000;
          }
          output.println(((msg_servo_output_raw) msg).servo1_raw + ";"
              + ((msg_servo_output_raw) msg).servo2_raw + ";"
              + ((msg_servo_output_raw) msg).servo3_raw + ";"
              + ((msg_servo_output_raw) msg).servo4_raw + ";;;"
              + ((double) (timeOutput - lastTimeOutput) / 1000));
        }
        if (msg != null
            && msg.messageType == IMAVLinkMessageID.MAVLINK_MSG_ID_RC_CHANNELS_RAW
          ...
          input.println(((msg_rc_channels_raw) msg).chan1_raw + ";"
