...
public class MAVLinkReader {
     ...
     * Return next message.
     * If bytes available, try to read it.
     * Don't wait message is completed, it will be retruned nex time
     * @return MAVLink message or null
     */
                ...
                if ((crcl == crcLow) && (crch == crcHigh)) {
                    ...
                    if (msg != null) {
                        ...
                        if (!checkSequence(sequence)) {
                            System.err.println("SEQUENCE error, packets lost! Last sequence : " + lastSequence + " Current sequence : " + sequence
    ...
    /**
     * Read the end of message after the start byte and the payload length.
     * Called only if there are available character to read all the rest of message
    ...
    protected boolean readEndMessage() throws IOException {
        ...
        if ((crcl == crcLow) && (crch == crcHigh)) {
            ...
            if (msg != null) {
                msg.sequence = sequence;
                if (!checkSequence(sequence)) {
                    System.err.println("SEQUENCE error, packets lost! Last sequence : " + lastSequence + " Current sequence : " + sequence + " Id="
            ...
            System.err.println("ERROR mavlink CRC16-CCITT compute= " + Integer.toHexString(crc) + "  expected : "
                               + Integer.toHexString(crcHigh & 0x00FF) + Integer.toHexString(crcLow & 0x00FF) + " Id=" + msgId + " nbReceived="
                               + nbReceived);
            validData = false;
        }
        // restart buffer
