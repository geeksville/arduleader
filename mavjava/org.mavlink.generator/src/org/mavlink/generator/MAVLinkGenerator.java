...
public class MAVLinkGenerator {
	...
	/**
	 * Main class for the generator. Command line arguments are : source :
	 * directory path containing xml files to parse for generation target :
	 * directory path for output Java source files isLittleEndian : true if type
	 * are stored in LittleEndian in buffer, false for BigEndian forEmbeddedJava
	 * : true if generated code must use apis for embedded code, false else
	 * useExtraByte : if true use extra crc byte to compute CRC debug : true to
	 * generate toString methods in each message class
	 * 
	 * Example : java org.mavlink.generator.MAVLinkGenerator resources/1.0
	 * target/ true true true true Generate MAVLink message Java classes for
	 * mavlink xml files contains in resources/1.0 in target diretory for Little
	 * Endian data, embedded code using extra byte for crc and generating debug
	 * code
	 * 
	 * @param args
	 */
	...
	public void usage(String[] args) {
		...
		System.out
				.println("  source : directory path containing xml files to parse for generation");
		System.out
				.println("  target : directory path for output Java source files");
		System.out
				.println("  isLittleEndian : true if type are stored in LittleEndian in buffer, false for BigEndian");
		System.out
				.println("  forEmbeddedJava : true if generated code must use apis for embedded code, false else");
		System.out
				.println("  useExtraByte : if true use extra crc byte to compute CRC");
		System.out
				.println("  debug : true to generate toString methods in each message class");
		System.out.println("  ");
		System.out.println("Example :");
		System.out
				.println("  java org.mavlink.generator.MAVLinkGenerator  resources/v1.0 target/ true true true");
		System.out
				.println("  Generate MAVLink message Java classes for mavlink xml files contains in resources/v1.0 ");
		System.out
				.println("  in target diretory for Little Endian data, embedded code with debug code");
	}
		...
		try {
			for (int i = 0; i < files.length; i++) {
				if (files[i].endsWith(".xml")) {
					...
					String rep = target + File.separator
							+ files[i].substring(0, index);
					parseFile(files[i], path, rep);
					// MAVLinkData mavlink = null;
					// mavlink = new MAVLinkData();
					//
					// Map<String, String> implementations = parseFile(mavlink,
					// files[i], path, rep, false);
					//
			...
			System.err
					.println("MAVLinkGenerator Error : " + file + "  =  " + e);
			e.printStackTrace();
		}
	}
	...
	protected void parseFile(String filename, String dir, String destination) {
		try {
			...
			System.out.println("Parse : " + filename + " in directory : " + dir
					+ " to directory :" + destination);
			MAVLinkData mavlink = null;
			mavlink = new MAVLinkData();
			Map<String, String> implementations = parseFile(mavlink, filename,
					dir, destination, false);
			...
			System.err.println("MAVLinkGenerator Error : " + filename + "  =  "
					+ e);
			e.printStackTrace();
		}
	}
	 ...
	 * @throws IOException
	 */
	public Map<String, String> parseFile(MAVLinkData mavlink, String file,
			String path, String target, boolean inInclude)
		...
		if (!inInclude) {
			...
			System.out.println("MAVLinkData INCLUDE : "
					+ file.substring(0, file.indexOf('.')));
		}
		MAVLinkHandler gestionnaire = new MAVLinkHandler(this, mavlink, path,
				target);
		...
		for (MAVLinkMessage message : mavlink.getMessages().values()) {
			...
			try {
				...
				for (int j = 0; j < message.getFields().size(); j++) {
					...
					fieldWrite.append("  public "
							+ type.getJavaType(field.getName()) + "\n");
					sbRead.append(type.getReadType(field.getName(),
							forEmbeddedJava));
					sbWrite.append(type.getWriteType(field.getName(),
							forEmbeddedJava));
					String attr = field.getName();
					if (type.isArray && type.type == MAVLinkDataType.CHAR) {
						String first = "" + attr.charAt(0);
						attr = first.toUpperCase()
								+ field.getName().substring(1);
						fieldWrite.append("  public void set" + attr
								+ "(String tmp) {\n");
						fieldWrite
								.append("    int len = Math.min(tmp.length(), "
										+ type.arrayLenth + ");\n");
						fieldWrite
								.append("    for (int i=0; i<len; i++) {\n      "
										+ field.getName()
										+ "[i] = tmp.charAt(i);\n    }\n");
						fieldWrite.append("    for (int i=len; i<"
								+ type.arrayLenth + "; i++) {\n      "
								+ field.getName() + "[i] = 0;\n    }\n  }\n");
					...
					forToString = forToString
							+ (j != 0 ? "+" : "")
							+ "  \"  "
							+ field.getName()
							+ "=\"+"
							+ (field.getType().isArray
									&& field.getType().type == MAVLinkDataType.CHAR ? "get"
									+ attr + "()"
									: field.getName());
					extraCrcBuffer = extraCrcBuffer + type.getCType() + " "
							+ field.getName() + " ";
					if (type.isArray) {
						extraCrcBuffer = extraCrcBuffer
								+ (char) type.arrayLenth;
					}
				}
				writer.print("    length = " + fieldLen + ";\n}\n\n");
				writer.print(fieldWrite.toString());
				int extra_crc = MAVLinkCRC.crc_calculate(MAVLinkCRC
						.stringToByte(extraCrcBuffer));
				int magicNumber = (extra_crc & 0x00FF)
						^ ((extra_crc >> 8 & 0x00FF));
				...
				writer.print("  int crc = MAVLinkCRC.crc_calculate_encode(buffer, "
						+ fieldLen + ");\n");
	 ...
	 */
	protected void generateEnumClass(MAVLinkData mavlink, String targetPath,
			...
			try {
				...
				for (int j = 0; j < message.getEntries().size(); j++) {
					...
					for (int k = 0; k < entry.getParams().size(); k++) {
						...
						writer.print("     * PARAM " + param.getIndex() + " : "
								+ param.getComment() + "\n");
					}
					writer.print("     */\n");
					int v = entry.getValue();
					if (v == -1)
						v = j; // If there was no value specified in the XML,
								// just use the index
					writer.print("    public final static int "
							+ entry.getName() + " = " + v + ";\n");
				}
				writer.print("}\n");
			} catch (Exception e) {
	 ...
	 */
	protected void generateMAVLinkClass(String targetPath,
		...
		try {
			...
			if (implementation.size() != 0) {
				writer.print("public interface " + className + " extends "
						+ allImpl.substring(0, allImpl.length() - 1)
						+ " {\n}\n");
			} else {
				writer.print("public interface " + className + " {\n}\n");
			}
	...
	protected void generateFactoryClass(MAVLinkData mavlink, String targetPath) {
		...
		try {
			...
			for (MAVLinkMessage message : mavlink.getMessages().values()) {
				...
				writer.print("      msg = new " + msgClassName
						+ "(sysId, componentId);\n");
	...
	protected void generateIMavlinkId(MAVLinkData mavlink, String targetPath) {
		...
		try {
			...
			for (MAVLinkMessage message : mavlink.getMessages().values()) {
				String id = MAVLINK_MSG + "_ID_" + message.getName();
				writer.print("  public static int " + id + " = "
						+ message.getId() + ";\n");
			}
			writer.print("}\n");
		} catch (Exception e) {
