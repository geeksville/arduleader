...
public class HUD extends SurfaceView implements SurfaceHolder.Callback {
  ...
  public HUD(Context context, AttributeSet attributeSet) {
    ...
    white
        .setTextSize(15.0f * context.getResources().getDisplayMetrics().density);
    ...
    statusText
        .setTextSize(25.0f * context.getResources().getDisplayMetrics().density);
  ...
  private void drawText(Canvas canvas, int i, String text, Paint p, boolean left) {
    ...
    if (left)
      canvas.drawText(text, (float) (-width / 2.0 + bounds.height() * .2f), y,
          p);
    else
  ...
  private void drawYaw(Canvas canvas) {
    ...
    for (double angle = (centerDegrees - mod) - numDegreesToShow / 2.0; angle <= (centerDegrees - mod)
      ...
      canvas.drawLine(distanceToCenter, -height / 2 + 20, distanceToCenter,
          -height / 2 + 40, white);

      if (workAngle % 90 == 0) {
        String compass[] = { "N", "E", "S", "W" };
        int index = (int) workAngle / 90;
        canvas.drawText(compass[index], distanceToCenter, -height / 2 + 20,
            whiteCenter);
